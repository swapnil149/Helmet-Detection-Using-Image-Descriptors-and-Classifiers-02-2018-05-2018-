# -*- coding: utf-8 -*-
"""
Created on Fri Apr 27 03:40:22 2018
...
path_crop='E:/images_veh_unscaled_crop/'
if not os.path.exists(path_crop):
    os.makedirs(path_crop)
#
#while count<len(myimages):
#    ig=cv2.imread(os.path.join(path,myimages[count]))
#    crop_img = ig[y:y+h]
#    cv2.imwrite(os.path.join(path_crop,myimages[count]), crop_img)
#    count+=1
...
#  
my_images = [] #list of image filenames
dirFiles = os.listdir(path_crop)
dirFiles.sort(key=lambda f: int(filter(str.isdigit, f)))
for files in dirFiles: #filter out all non jpgs
...
dic={}
print(len(my_images))
while count<len(my_images):
#while count<3000:
    #vehicle_image= cv2.imread(os.path.join(path,'8274_1'+'.png'))
    helmet= cv2.imread(os.path.join(path_crop,my_images[count]),cv2.IMREAD_GRAYSCALE)
...
#    vehicle_features = source.features(vehicle_image)
    ...
    dic[my_images[count]]=helmet_features
    count+=1
    if count%1000==0:
        print(count)
#print dic

with open("E:/hog_dict_helmet.pkl",'wb') as f:
